...
from vk_service.factories.group.group_list_factory import GroupListFactory
...
class GroupListFactoryTest(unittest.TestCase):
    def setUp(self):
        self.factory = GroupListFactory()
        ...
        self.values = [self.group.copy(), self.group.copy(), self.group.copy()]
        self.submodels = {
            'ban_info': Ban,
            'city': City,
            'contacts': Contact,
    ...
    def test_factory_build(self):
        ...
        groups = self.factory.build(values)
        self.assertIsInstance(groups, list)
        for i, group in enumerate(groups):
            self.assertIsInstance(group, Group)
            for key in self.submodels:
                values[i][key] = group.__dict__[key]
            for key in self.submodels:
            ...
            self.assertEqual(group.__dict__, values[i])
